...
function SafeBookCover({ src, alt }: { src?: string; alt: string }) {
  ...
  return (
    <div
      className={`w-full h-full bg-background-muted flex items-center justify-center transition-opacity duration-200 ${isLoaded ? 'opacity-100' : 'opacity-40'}`}
    >
      <img
        src={imageSrc}
...
export default function SearchResultsPage() {
  ...
  const { data: searchResult, isLoading: isSearching } =
    useBorrowBookSearch(searchQuery)
  ...
  const externalMatches = Array.isArray(searchResult?.externalData)
    ? searchResult.externalData
        .map((b: any) => {
          const normalised = normaliseBookPayload(
            b,
            searchResult.externalSource || 'openlibrary',
          )
          return {
            ...normalised,
            isLocal: false,
  ...
  if (isSearching) {
    return (
      <div className="p-6 max-w-app mx-auto text-text-muted">
        Searching the library inventories...
      </div>
    )
  }
    ...
    <div className="p-6 max-w-app mx-auto">
      <div>
        ...
        <p className="text-xs text-text-muted mt-1">
          Found {localMatches.length} local items and {externalMatches.length}{' '}
          global references.
        </p>
      </div>
        ...
        {localMatches.length > 0 ? (
          <ul className="divide-y divide-border">
            {localMatches.map((book: SelectableBook) => (
              <li
                key={book.id}
                className="flex gap-4 py-3 items-center justify-between"
              >
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    <a
                      ...
                      className="font-bold text-sm text-secondary hover:underline"
                    >
        ...
        <div
          className={`border rounded-sm p-4 transition-all duration-200 ${
            isExpanded
              ...
              : 'bg-background-muted/40 border-border/60 opacity-70'
          }`}
        >
          ...
          {isExpanded && (
            <div className="mt-4 pt-4 border-t border-border/60 animate-fadeIn">
              <p className="text-xs text-text-muted mb-3 italic">
                These books are not yet in our system, but might be available in
                another library:
              </p>
              <ul className="divide-y divide-border opacity-90">
                {externalMatches.map((book: any, i: number) => {
                  ...
                  return (
                    <li
                      key={`external-${book.work_id || 'no-work'}-${book.isbn || 'no-isbn'}-${i}`}
                        ...
                        <div>
                          {itemLinkUrl ? (
                            <a
                              ...
                              className="font-medium text-sm text-text-primary hover:underline flex items-center gap-1"
                            >
                              {book.title}{' '}
