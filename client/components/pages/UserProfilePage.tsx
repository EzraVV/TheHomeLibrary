...
export default function UserProfilePage() {
  ...
  if (!user) {
    return (
      <div className="w-full px-4 py-12 text-center flex flex-col items-center justify-center gap-4">
          <div className="p-8 bg-surface rounded-md shadow-card border border-border/40 max-w-md w-full py-12">
            <LogIn aria-hidden="true" className="w-12 h-12 text-text-muted/40 mx-auto mb-3" />
  ...
  return (
    <div className="w-full flex flex-col gap-8 text-left">
      ...
      <UserAbout />
      <section
        aria-labelledby="profile-book-sections"
        className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start"
      >
        <h2 id="profile-book-sections" className="sr-only">
          Your book activity
        </h2>
        <UserBooksOwned />
        <UserBooksBorrowed />
